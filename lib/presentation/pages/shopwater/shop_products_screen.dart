...
import 'package:riverpodtemp/application/shopwater/shop_provider.dart';
  ...
  void initState() {
    tabController = TabController(
        length: (widget.listCategory?.length ?? 0) +
            (ref.read(shopWaterProvider).popularProducts.isNotEmpty ? 1 : 0),
        vsync: this)
      ..addListener(() {
      ...
      children: [
        SizedBox(
          ...
          child: makeTabBarHeader(
            ...
            isPopularProduct: ref.watch(shopWaterProvider).isPopularProduct,
        ...
        Expanded(
            child: SingleChildScrollView(
              child: (!ref.watch(shopWaterProvider).isProductLoading &&
                  ref.watch(shopWaterProvider).products.isEmpty)
                  ?  _resultEmpty():Column(
                children: [
                  VisibilityDetector(
